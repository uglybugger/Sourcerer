...
namespace Sourcerer.DomainConcepts
{
    public class Repository<T> : IRepository<T> where T : class, IAggregateRoot
    {
        ...
        public Repository(ISnapshot<T> snapshot, IUnitOfWork unitOfWork)
        {
            _snapshot = snapshot;
        ...
        {
            var item = _modifiedItems.GetOrAdd(id, itemId => _snapshot.GetById(itemId).Clone<T>());
        ...
        public T[] Query(IQuery<T> query)
        {
            ...
            var fromSnapshot = query.Execute(_snapshot.Items)
                                    .Except(fromUoW)
                                    .Except(_removedItems.Values)
                                    .AsParallel()
        ...
        {
            return query.Execute(_snapshot.Items);
        }

        private void OnUnitOfWorkCompleted(object sender, EventArgs e)
        {
            _snapshot.UpdateAtomically(_newItems.Values, _modifiedItems.Values, _removedItems.Values);
        }
